...
mounted=''
lib::secret::read_file mounted /run/secrets/certificate
...
if lib::secret::read_file denied /run/secrets/denied; then exit 1; fi

export REPO_ROOT=/opt/libsh-test
bash "$REPO_ROOT/test/bats/core/bin/bats" "$REPO_ROOT/test/lib/secret.bats" "$REPO_ROOT/test/lib/networking.bats" "$REPO_ROOT/test/bin"
