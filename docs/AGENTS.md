...
A Bash 4.0+ (MIT) library with four shipped bundles:
...
- `make test` — all BATS tests; `make test WHAT=lib` or `WHAT=bin` for one directory
- Single test file: `test/bats/core/bin/bats test/lib/opts.bats` (a `bats` on `PATH` is preferred; `make test` falls back to the submodule)
...
- `lib/` modules: first line `# shellcheck shell=bash`; public API
  ...
  (`<module>` = filename without `.sh`). Every module needs `test/lib/<module>.bats`,
  and `lib/lib.sh` must expose every function.
...
- Scripts parse args via `lib::opts::parse`: define `OPTS`, `declare -A OPTS_HELP`,
...
- Never edit the `Makefile` `VERSION` by hand. semantic-release on `main`/`next`
  ...
  the archives. `fix:` bumps PATCH, `feat:` MINOR, `BREAKING CHANGE:` MAJOR.
