...
# Globals: None
# Arguments: VERSION (no leading v or surrounding whitespace)
...
# Globals: None
# Arguments: VERSION FIELD (major|minor|patch|prerelease|build)
# Outputs: Field and newline on stdout; absent prerelease/build prints an empty
...
# Arguments: INTERVAL ATTEMPTS -- COMMAND [ARG...]
...
# Returns: 0 command succeeded, 1 all attempts failed or sleep failed,
...
function lib::core::retry() {
  ...
  if [[ $# -lt 4 || ${3:-} != -- || -z ${4:-} ]] ||
    ! __libsh_core_interval=$(__libsh_data_uint "$1") || ! __libsh_core_attempts=$(__libsh_data_uint "$2") ||
...
# Globals: Named scalar store (read), never modified.
# Arguments: STORE read|write
# Outputs: Validated store contents on stdout; sanitized errors on stderr.
# Returns: 0 valid (unset/empty is an empty store), 2 invalid reference/content.
#######################################
...
# Arguments: STORE ID STATUS (pending|success|failure)
#   Declare a local scalar first to keep the store local. Treat its serialization
...
# Arguments: STORE [STATUS (pending|success|failure)]
