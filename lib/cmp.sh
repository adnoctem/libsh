...
# Globals: None
# Arguments: LEFT RIGHT (already validated; no leading zeros except zero)
# Outputs: -1, 0 or 1 and newline.
# Returns: 0 success.
#######################################
...
#######################################
# Compare complete SemVer 2.0.0 versions by precedence, ignoring build metadata.
...
# Globals: None
# Arguments: LEFT_BYTES RIGHT_BYTES (nonnegative, within signed 64-bit range)
...
# Globals: PATH (read)
# Arguments: PATH file|directory|link
# Outputs: Mode, UID, GID, mtime seconds, and (except directories) logical size.
# Returns: 0 success, 2 metadata failure.
#######################################
...
# mtime. Ignores contents, inode, atime, ctime, ACLs, xattrs, and subsecond times.
...
# Globals: PATH (read)
# Arguments: LEFT RIGHT (explicit symlinks followed)
...
function lib::cmp::file_diff() {
  ...
  [[ $left == /* ]] || left=./$left
  [[ $right == /* ]] || right=./$right
  if diff -u -- "$left" "$right"; then return 0; else status=$?; fi
  [[ $status == 1 ]] && return 1
...
# Globals: None
# Arguments: PATH
# Outputs: link, directory, file, or error on stderr.
# Returns: 0 classified, 2 missing/unsupported entry (FIFO/socket/device).
#######################################
...
# Globals: None (requires dotglob/nullglob in caller's subshell)
# Arguments: DIRECTORY attributes|contents
# Outputs: Errors stderr.
# Returns: 0 inspectable, 2 inaccessible or unsupported tree.
#######################################
...
# Globals: PATH (read)
# Arguments: LEFT RIGHT
# Outputs: None; errors stderr.
# Returns: 0 identical link text, 1 different, 2 readlink failure.
#######################################
...
# Globals: None (requires dotglob/nullglob and LC_ALL=C in caller's subshell)
# Arguments: LEFT RIGHT attributes|contents
# Outputs: Content diffs/escaped mismatch paths in contents mode; errors stderr.
# Returns: 0 equal, 1 different, 2 inspection failure (takes precedence).
#######################################
function __libsh_cmp_tree() {
  ...
  for entry in "$1"/*; do
    ...
    case $left_type in
    directory)
      if __libsh_cmp_tree "$entry" "$other" "$3"; then result=0; else result=$?; fi
      ;;
    link)
      if __libsh_cmp_links "$entry" "$other"; then result=0; else result=$?; fi
    ...
    file)
      if [[ $3 == contents ]]; then
        if lib::cmp::file_diff "$entry" "$other"; then result=0; else result=$?; fi
      else
        result=0
      fi
      ;;
...
# Globals: None; cwd, options, locale and traps are preserved in the caller.
# Arguments: attributes|contents LEFT RIGHT
# Outputs: As __libsh_cmp_tree.
# Returns: 0 equal, 1 different, 2 invalid or inaccessible inputs.
#######################################
...
# Globals: PATH (read)
# Arguments: LEFT RIGHT (explicit directory roots may be symlinks)
# Outputs: Errors stderr; no comparison data on stdout.
...
# Ignores directory inode sizes. Compares symlink text without following links;
# counts hard-linked paths independently; crosses mounts; includes hidden files.
# Excludes file contents, ACLs, xattrs and subsecond timestamps. Not a snapshot.
...
# Globals: PATH (read)
# Arguments: LEFT RIGHT (explicit directory roots may be symlinks)
# Outputs: Unified file diffs and escaped path/type/link mismatches on stdout;
...
# ignores ownership, modes, times and hardlink topology. Never opens special files.
